...
`arm64`, are attached to each
[GitHub Release](https://github.com/syndr/swaylock-plugin/releases),
for both `swaylock-plugin` and `windowtolayer` (the screensaver lockscreen
setup needs both). Download the files matching your architecture and distro
(the name carries both, e.g.
`swaylock-plugin_1.8.6.1-1_arm64.debian13.deb`) and install them
with apt:
...
`swaylock-plugin-screensaver` is `Architecture: all`, so its single `_all`
`.deb` installs on either architecture.
